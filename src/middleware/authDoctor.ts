...
// @access  Private
const verifyUser = (decodedToken: DecodedToken): Promise<Doctor | null> => {
...
const protectdoctor = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.headers.authorization) {
    try {
      ...
      const decoded = jwt.verify(accessToken as string, process.env.JWT_KEY_SECRET as string) as DecodedToken;
      verifyUser(decoded)
        .then((user) => {
          if (user) {
            if (!user.is_blocked) {
